...
#===========================================================================================================
    ...
    def conditional_GMM(self, in_dims, out_dims, value=None): 
    """compute the conditional probability of each y given x"""
        gmminf = gmr.gmm.sub_gmm(self.n_components) #probably miss a lign distringuish out/in
        if in_dims == 6 : #to change !!!
            conditional_gmm = local_gmminf.condition([0], value) #gmm de x |y
        else:
            conditional_gmm = local_gmminf.condition([1], value) #gmm de x |y
        return conditional_gmm
        
    def probility_density(self, in_dims, out_dims, value=None):
        proba = self.conditional_GMM(in_dims, out_dims, value).to_probability_density(value)       
        return proba
        
    def lse(self, in_dims, out_dims, value=None):
    """LSE is the least square estimate in order to compute the most probably value knowing an input value. """
        return self.conditional_GMM(in_dims, out_dims, value).weights.dot(conditional_GMM(in_dims, out_dims, value).means)
        
    def slse(self, in_dims, out_dims, value=None):
    """ SLSE is the S LSE and compute the most probably value of the most important gaussian knowing an input value"""
        max_weight = np.max(self.conditional_GMM(in_dims, out_dims, value).weights)
        max_ind = np.where(self.conditional_GMM(in_dims, out_dims, value).weights == max_weight)[0][0]
        return self.conditional_GMM(in_dims, out_dims, value).weights.means[max_ind]  
        
   def jonathanSampling(self, in_dims, out_dims, value=None): #don't know how to name it
   
   def stochasticSampling(self, in_dims, out_dims, value=None):
