...
class Imle(object):
    def __init__(self,  imle_path=imle_path, **kwargs):
        ...
        args = []
        MAX_NUMBER_OF_SOLUTIONS = 20
        # args.append(f('accelerated', False))
        args.append(f('alpha', 0.999))
        args.append(f('Psi0', [1.] * self.D))
        args.append(f('sigma0', 1.0))
        args.append(f('wsigma', 2 ** self.d))
        args.append(f('wpsi', 2 * self.D))
        args.append(f('wNu', 0.0))
        args.append(f('wLambda', 0.1))
        args.append(f('wSigma', 2.0 ** self.d))
        args.append(f('wPsi', 2.0 ** self.d))
        args.append(f('nOutliers', 1))
        args.append(f('p0', 0.1))
        args.append(f('multiValuedSignificance', 0.99))
        args.append(f('nSolMin', 1))
        args.append(f('nSolMax', MAX_NUMBER_OF_SOLUTIONS))
        args.append(f('iterMax', 2))
